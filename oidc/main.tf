module "oidc_server" {
  ...
  #server config
  server_name     = "oidc-${count.index + 1}"
  ...
  // e.g., for 10.50.1.26 use offset 26
  host_offset = var.host_offset_oidc
...
module "oidc_firewall" {
  source = "../modules/oidc_firewall"

  oidc_ssh_allowed_cidrs = local.oidc_ssh_allowed_cidrs
}
